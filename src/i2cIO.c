...
void gets_I2C(UINT8 *ucRdptr, UINT16 usLength, BOOL bAdjust){
	...
	if (!bAdjust){
		ret = i2c_smbus_read_i2c_block_data	(SLAVE_FD, (UINT8)0x00, usLength, ucRdptr); // you specify explicitly the length
	} else{
		ret = i2c_smbus_read_block_data		(SLAVE_FD, (UINT8)0x00, ucRdptr); //automatically get the number of bytes to read (up to 32bytes)
	}      
